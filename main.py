...
def stock_query():
    ...
    if stock_id.isdigit():
        ...
        if int(stock_id) in table['股票代號'].values:
            ...
            return render_template('search.html', stock_name = '您查詢的公司不存在', result = '請確認輸入的股票名稱或代號是否正確，再重新查詢看看') # 此股票不存在
    ...
    return render_template('search.html', stock_name = stock_name, stock_id = '(' + stock_id + ')', 
                            ...
                            Growth = Growth, EPS = EPS, D = D, price_GGM = price_GGM, price_SPM = price_SPM,
                            )
